...
export const userMustOwnReview = (req, res, next) => {
...
export const reviewIdMustBeValid = async (req, res, next) => {
    const reviewId = req.params.reviewId;
    try {
        const reviewDoesExist = await Review.exists(reviewId);
        if (reviewDoesExist) {
            next();
        } else {
            res.status(404);
            res.json({
                errors: [
                    "This ID does not match any existing reviews."
                ]
            })
        }
    } catch (error) {
...
export const userMustOwnComment = async (req, res, next) => {
    const commentId = req.params.commentId;
    const userId = req.session.currentUser._id;
    const [userIsOwner, error] = await commentDao.userOwnsComment(userId, commentId);
    if (error) {
        res.status(500);
        res.json({
            errors: [
